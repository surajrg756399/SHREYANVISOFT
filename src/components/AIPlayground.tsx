...
export default function AIPlayground() {
  ...
  // Verification State
  const [verifyStep, setVerifyStep] = useState(1); // 1: Email, 2: OTP
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/otp/send", {
        ...
        body: JSON.stringify({ email: userEmail, type: 'customer' })
      });
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setVerifyError("Verification failed. Contact sales@shreyanvisoft.com for assistance.");
    } finally {
      setIsVerifying(false);
    }
  };
          ...
          <AnimatePresence mode="wait">
            {!isAIVerified && (
              <motion.div 
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="ai-blur-overlay z-50"
              >
                <div className="bg-slate-900/95 backdrop-blur-xl p-10 rounded-[2.5rem] border border-white/10 text-center max-w-md w-full shadow-[0_50px_100px_rgba(0,0,0,0.5)]">
                  ...
                  <p className="text-xs text-slate-400 mb-8 font-medium tracking-wide">Enter your business email to receive a secure access code. <br/> <span className="text-cyan-400/80 mt-1 block italic">Example: yourname@company.com</span></p>
                  ...
                  {verifyStep === 1 ? (
                    <form onSubmit={handleSendOtp} className="space-y-4">
                      ...
                      >
                        {isVerifying ? 'Sending Code...' : 'Send Access Code'}
                      </button>
                    </form>
                  ) : (
                    <form onSubmit={handleVerifyOtp} className="space-y-4">
                      <div className="relative">
                        <input 
                          ...
                          maxLength={6}
                          className="w-full bg-slate-800/50 border border-slate-700 rounded-2xl px-5 py-4 text-sm text-white focus:border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-500/10 transition-all font-bold tracking-[0.5em] text-center" 
                        />
                      </div>
                      <button 
                        type="submit" 
                        disabled={isVerifying} 
                        className="w-full bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 font-black py-4 rounded-2xl text-xs tracking-[0.2em] uppercase transition-all shadow-lg disabled:opacity-50 active:scale-[0.98]"
                      >
