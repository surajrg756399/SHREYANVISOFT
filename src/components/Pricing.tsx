...
export default function Pricing() {
  ...
  const formatPriceStr = (valStr: string) => {
    if (!valStr) return "";
    const { symbol, rate, feeMultiplier } = exchangeRates[currency];
    ...
    if (valStr.includes('-')) {
      ...
      return `${formatNum(min, symbol, rate, feeMultiplier)} – ${formatNum(max, symbol, rate, feeMultiplier)}`;
    }
    
    // Check if it has a plus like "25,500+"
    const hasPlus = valStr.includes('+');
    const num = parseInt(valStr.replace(/,/g, '').replace(/\+/g, ''));
    return `${formatNum(num, symbol, rate, feeMultiplier)}${hasPlus ? '+' : ''}`;
  };

  const formatNum = (val: number, symbol: string, rate: number, feeMultiplier: number) => {
    if (isNaN(val)) return "";
    const withFees = val * rate * feeMultiplier;
    if (currency === 'INR') {
  ...
  // Helper for static numbers that aren't in KV yet
  const formatPrice = (val: number, plus = false) => {
    const { symbol, rate, feeMultiplier } = exchangeRates[currency];
    return `${formatNum(val, symbol, rate, feeMultiplier)}${plus ? '+' : ''}`;
  };
