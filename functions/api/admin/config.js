      ...
      if (body.admins) {
        ...
        if (isSuperAdmin) {
          ...
          const updatedAdmins = body.admins.map(newAdmin => {
            const existing = currentAdmins.find(a => a.email.toLowerCase() === newAdmin.email.toLowerCase());
            return {
              ...newAdmin,
              password: newAdmin.password !== undefined && newAdmin.password !== "" ? newAdmin.password : (existing ? existing.password : "")
            };
          });
          
          // Ensure at least one superadmin remains (either in KV or via env var)
          const hasSuperAdmin = updatedAdmins.some(a => a.role === 'superadmin') || !!env.ADMIN_EMAIL;
          if (!hasSuperAdmin) {
